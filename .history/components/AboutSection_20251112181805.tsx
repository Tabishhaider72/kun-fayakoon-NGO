...
export default function AboutSection() {
  ...
  // Detect scroll visibility for fade-in + video play
  useEffect(() => {
  ...
  useEffect(() => {
    if (videoRef.current) {
      if (isInView) {
        ...
        videoRef.current
          .play()
          .catch(() => console.warn("Autoplay prevented, waiting for user gesture."));
      } else {
        ...
        <div className="lg:col-span-6 flex justify-center">
          ...
          >
            <video
              ...
              playsInline
              preload="auto"
              poster="/about-thumb.png"
              className="rounded-2xl w-full h-auto object-cover transition-transform duration-300 hover:scale-[1.02]"
            >
              <source src="/about-video.mp4" type="video/mp4" />
              Your browser does not support the video tag.
            </video>

            {/* Play icon overlay (fades away while playing) */}
            <motion.div
              initial={{ opacity: 1 }}
